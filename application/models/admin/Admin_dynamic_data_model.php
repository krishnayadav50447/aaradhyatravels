...
class Admin_dynamic_data_model extends CI_Model{
    /************BLOG****************/
    ...
    public function add_blog($data){
        return $this->db->insert('webina_blog', $data);
    }
    ...
    public function update_blog($id, $data){
        if(array_key_exists('img', $data)){
            ...
            $temp_img=$this->db->select('img')->where('id', $id)->get('webina_blog')->row()->img;
            if(!empty($temp_img)){
    ...
    public function blog_status_data($id, $status){
        return $this->db->where("id", $id)->update('webina_blog', array('status'=>$status));
    }
    ...
    public function add_faq($data){
        return $this->db->insert('webina_faq', $data);
    }
    public function update_faq_fetch($id){
        return $this->db->where('id', $id)->get('webina_faq')->row_array();
    }
    public function update_faq($id, $data){
        return $this->db->where('id', $id)->update('webina_faq', $data);
    }
    public function faq_status_data($id, $status){
        return $this->db->where("id", $id)->update('webina_faq', array('status'=>$status));
    }
    public function delete_faq($id){
        if($this->db->where('id', $id)->delete('webina_faq')){
            return array('type'=>'success','text'=>'Deleted');
        }else{
            return array('type'=>'warning','text'=>'Error..!');
        }
    ...
    public function fetch_all_clientele(){
        ...
        $search = trim(strip_tags($this->input->post("search")['value']));
        $this->db->select('a.*');
        $this->db->from('webina_clientele a');
        if($search!=""){
            $this->db->where( "(`a`.`name` LIKE '%$search%' ESCAPE '!' OR `a`.`description` LIKE '%$search%' ESCAPE '!')");
        }
        $data = $this->db->limit($length, $start)->order_by('id', 'desc')->get()->result();
    ...
    public function add_clientele($data){
        return $this->db->insert('webina_clientele', $data);
    }
    public function update_clientele_fetch($id){
        return $this->db->where('id', $id)->get('webina_clientele')->row_array();
    }
    public function update_clientele($id, $data){
        if(array_key_exists('img', $data)){
            $folder='./uploads/media/';
            $temp_img=$this->db->select('img')->where('id', $id)->get('webina_clientele')->row()->img;
            if(!empty($temp_img)){
                unlink($folder.$temp_img);
            }
        }
        return $this->db->where('id', $id)->update('webina_clientele', $data);
    }
    public function clientele_status_data($id, $status){
        return $this->db->where("id", $id)->update('webina_clientele', array('status'=>$status));
    }
    public function delete_clientele($id){
        ...
        if($this->db->where('id', $id)->delete('webina_clientele')){
            return array('type'=>'success','text'=>'Deleted');
        }else{
            return array('type'=>'warning','text'=>'Error..!');
        }
